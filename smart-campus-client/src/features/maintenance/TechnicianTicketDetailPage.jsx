...
const TechnicianTicketDetailPage = () => {
  ...
  const { getTicket, startWork, resolveTicket, addTicketComment, updateWorkProgress } = useContext(TicketContext);
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      try {
        ...
        setProgressValue(data.workProgress ?? 0);
      } catch (error) {
  ...
  const commitProgress = async (nextValue) => {
    const safeProgress = clampProgress(nextValue);

    if (!ticket || safeProgress === (ticket.workProgress ?? 0)) {
    ...
    try {
      const updatedTicket = await updateWorkProgress(id, safeProgress);
      setTicket(updatedTicket);
      setProgressValue(updatedTicket.workProgress ?? safeProgress);
    } catch (error) {
      setProgressValue(ticket.workProgress ?? 0);
  ...
  const handleStartWork = async () => {
    try {
      ...
      setTicket(updatedTicket);
      setProgressValue(updatedTicket.workProgress ?? 0);
    } catch (error) {
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      setTicket(updatedTicket);
      setProgressValue(updatedTicket.workProgress ?? 100);
    ...
    <div className="max-w-6xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <h2 className="text-lg font-bold text-gray-800 mb-6">Comments & Updates</h2>
            ...
            <div className="flex justify-between items-end mb-2">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    commitProgress(progressValue);
                  }
                }}
                className="w-20 rounded-lg border border-orange-200 bg-orange-50 px-2 py-1 text-right text-sm font-bold text-orange-500 outline-none"
              />
            </div>

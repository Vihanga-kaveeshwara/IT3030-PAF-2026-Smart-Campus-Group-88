// Frontend: src/features/maintenance/AdminTicketManagementPage.jsx
...
const AdminTicketManagementPage = () => {
  ...
  const { getTicket, assignTicket, updateStatus, rejectTicket } = useContext(TicketContext);
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      // Assuming priority update goes with patch if needed. Since prompt only said "update status", we'll just handle status & assign for now.
      alert('✅ Ticket updated successfully!');
      navigate('/maintenance/admin'); // route back to dashboard
    } catch (err) {
  ...
  // Dummy comments for UI simulation
  const dummyComments = [
    { sender: "Admin", role: "System Administrator", text: "Ticket received and assigned.", time: "Apr 14, 11:00 AM" },
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
                ...
                <div key={i} className="relative flex items-start gap-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl flex-1 border border-gray-100">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <span className="font-bold text-gray-800">{c.sender}</span>
            ...
            <textarea 
              ...
              rows="3"
            ></textarea>
            <button className="bg-[#053769] text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-[#042d55] transition">Post Comment</button>
          </div>
        </div>
